...
func vmFileSeek(vm *ligo.VM, a ...ligo.Variable) ligo.Variable {
	if len(a) != 3 {
		vm.Throw("file-seek : expected 3 arguments exactly")
	}
	if a[0].Type != 0x10080 {
		vm.Throw("file-seek : not a valid file handler")
	}

	if a[1].Type != ligo.TypeInt {
		vm.Throw("file-seek : position should be an integer")
	}

	if a[2].Type != ligo.TypeInt {
		vm.Throw("file-seek : whence should be an integer")
	}
	...
	if err != nil {
		vm.Throw("Error while seeking : " + err.Error())
	}
...
func vmFileClose(vm *ligo.VM, a ...ligo.Variable) ligo.Variable {
	if len(a) != 1 {
		vm.Throw("file-close : expected 1 arguments exactly")
	}
	if a[0].Type != 0x10080 {
		vm.Throw("file-close : not a valid file handler")
	}
...
func vmFileOpen(vm *ligo.VM, a ...ligo.Variable) ligo.Variable {
	if len(a) != 2 {
		vm.Throw("file-open : expected 2 arguments exactly")
	}

	f := a[0]
	if f.Type != ligo.TypeString {
		vm.Throw("file-open : expects a filename as a string")
	}
	filename := f.Value.(string)
	if a[1].Type != ligo.TypeString {
		vm.Throw("file-open : expects a mode as a string")
	}
	...
	case "r":
		...
		if err != nil {
			vm.Throw(fmt.Sprintf("file-open : error occurred : %s", err))
		}
	case "w":
	...
	default:
		vm.Throw("file-open : unrecogonized mode \"" + mode + "\"")
	}
...
func vmFileRead(vm *ligo.VM, a ...ligo.Variable) ligo.Variable {
	if len(a) != 2 {
		vm.Throw("file-read : expected 2 arguments exactly")
	}
	if a[0].Type != 0x10080 {
		vm.Throw("file-read : not a valid file handler")
	}

	fh := a[0].Value.(*os.File)

	if a[1].Type != ligo.TypeInt {
		vm.Throw("file-read : not a valid character count")
	}
	...
	if err != nil && err != io.EOF {
		vm.Throw(fmt.Sprintf("file-read : error occurred while reading : %s", err))
	}
...
func vmFileWrite(vm *ligo.VM, a ...ligo.Variable) ligo.Variable {
	if len(a) != 2 {
		vm.Throw("file-write : expected 2 arguments exactly")
	}
	if a[0].Type != 0x10080 {
		vm.Throw("file-write : not a valid file handler")
	}

	fh := a[0].Value.(*os.File)

	if a[1].Type != ligo.TypeString {
		vm.Throw("file-write : not a valid string")
	}
	...
	if err != nil {
		vm.Throw(fmt.Sprintf("file-write : error occurred while writing : %s", err))
	}
